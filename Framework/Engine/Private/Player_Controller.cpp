...
void CPlayer_Controller::Tick(_float fTimeDelta)
{
	if (Get_Enable())
	{
		...
		if (m_fTime < 0.f)
		{
			...
			m_pMyObject->Get_Component<CTargeting>()->Make_TargetList_Cursor(GAMEINSTANCE->Find_Layer(CURRENT_LEVEL, TEXT("Enemy_TargetBoard")), m_pMyObject->Get_Component<CTransform>());
			m_fTime = 1.f;
		}
