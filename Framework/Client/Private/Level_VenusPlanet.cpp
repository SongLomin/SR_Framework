...
void CLevel_VenusPlanet::VenusPlanet_Event(_float fTimeDelta)
{
	...
	if (m_fTextBoxTime <= 284.f && !m_bEventCheck[3])
	{
		...
		for (int i = 0; i < 3; ++i)
		{
			CTransform* pEnemyTransform = GAMEINSTANCE->Add_GameObject<CEnemy_TargetBoard>(CURRENT_LEVEL, TEXT("Enemy_TargetBoard"), nullptr, nullptr, true)->Get_Component<CTransform>();
